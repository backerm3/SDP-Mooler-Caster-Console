...
public class LibraryEditor {
	...
	public LibraryEditor(String oldName) {
		...
		RadioButton mrs = new RadioButton("LER MRS");
		fileSystem.setToggleGroup(typeOpts);
		csv.setToggleGroup(typeOpts);
		mrs.setToggleGroup(typeOpts);
		libTypeSel.getChildren().addAll(new Label("Library Type:"), fileSystem, csv, mrs);
		...
		if ( oldName != null ) {
			...
			else if ( lib instanceof MRSLibrary ) {
		...
		ok.setOnAction((evt) -> {
			...
			if ( fileSystem.isSelected() ) {
				...
				stage.close();
				LibraryManager.putLibrary(name.getText(), oldName, new MRSLibrary(name.getText(), location.getText(), flags));
				LibraryManagerUI.refreshList();
			}
			else {
